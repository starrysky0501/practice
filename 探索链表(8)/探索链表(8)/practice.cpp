...
//		//将链表形成环状。
//		cur->next = head;
//		//寻找新链表的头节点的前一个节点，也就是新链表的尾节点。
//		cur = head;
//		int m = count - len - 1;        //-1是为了寻找尾节点
//		while (m--)
//		{
//			cur = cur->next;
//		}
//		ListNode *new_head = cur->next;       //标记新的头节点
//		cur->next = nullptr;        //将尾节点的next指针域置空    
//		return new_head;
//	}
//};
//该方法思路：
//我们将原链表形成环状。然后找到断点位置以及新的头节点。
//请注意：移动K个位置，其实就是将链表中的后K个节点"整体"移动到原链表的头部。
//根据画图验证可知，第n-k个节点为新链表的头节点。(k<n)
//当k>n时，则第(n-k%n)个节点为新链表的头节点。
